...
class GeminiService
{
    ...
    public function generateResponse(string $message, string $userId = null): ?string
    {
        if (!$this->isEnabled()) {
            ...
            return "Sistem AI sedang dalam perbaikan. Silakan coba lagi nanti.";
        }
        ...
        try {
            ...
            return "Sistem AI mengalami masalah format. Silakan coba lagi.";

        } catch (RequestException $e) {
            if ($e->getResponse() && $e->getResponse()->getStatusCode() === 429) {
                Log::info('Gemini API rate limit hit, quota exhausted', ['user_id' => $userId]);
                return "Maaf, sistem sedang sibuk. Silakan tunggu beberapa detik dan coba lagi.";
            }
    ...
    private function getSystemInstruction(): string
    {
        ...
        $defaultInstruction = "You are an AI assistant for an AI Farm management system. Your role is to help users with questions about livestock farming, animal health, breeding, feeding, and farm management. Respond in Indonesian language (Bahasa Indonesia).";
    ...
    public function testConnection(): array
    {
        ...
        try {
            ...
            if ($response && !$response->startsWith('Sistem')) {
                return [
                    'success' => true,
