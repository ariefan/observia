...
interface Props {
  ...
  errorLevels?: Array<{ value: string; label: string }>;
...
const search = () => {
  router.get(route('error-logs.index'), filters.value, {
    preserveState: true,
    replace: true,
  });
};
...
const exportLogs = () => {
  window.open(route('error-logs.export', filters.value), '_blank');
};
...
<template>
  ...
  <AppLayout>
    ...
    <div class="flex min-h-screen">
      <!-- Sidebar -->
      <SecondSidebar current-route="error-logs.index" />

      <div class="flex-1 flex flex-col gap-4 p-4 max-w-7xl mx-auto space-y-6">
      <!-- Header -->
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Riwayat Error</h1>
          <p class="text-muted-foreground">
            {{ selectedFarmObject?.name === 'Semua Peternakan' 
                ? 'Lacak error dan masalah di seluruh peternakan' 
                : `Lacak error dan masalah di ${selectedFarmObject?.name}` }}
          </p>
        </div>
        <div class="flex items-center gap-3">
          <!-- Farm Selection Combobox for Super Users -->
          <Combobox v-model="filters.farm_id" @update:model-value="handleFarmChange">
            <div class="relative">
              <ComboboxButton
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Filter riwayat error berdasarkan level, tanggal, atau kata kunci
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="space-y-2">
              ...
              <div class="relative">
                ...
                <Input
                  ...
                  placeholder="Cari pesan error, file, pengguna..."
                  class="pl-8"
                  @keyup.enter="search"
                />
              </div>
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                <Label for="level">Level Error</Label>
                <Button 
                  v-if="filters.level" 
              ...
              <Select v-model="filters.level">
                <SelectTrigger>
                  <SelectValue placeholder="Pilih level error" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem 
                    v-for="errorLevel in (errorLevels || [])" 
        ...
        <CardHeader>
          <CardTitle class="text-lg">
            ...
            <span class="text-sm font-normal text-muted-foreground ml-2">
              ({{ errorLogs.data?.length || 0 }} catatan)
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div class="rounded-md border">
            <Table>
              ...
              <TableBody>
                <TableRow v-for="log in (errorLogs.data || [])" :key="log.id">
                  ...
                  <TableCell>
                    <div class="max-w-md">
                      <span class="text-sm" :title="log.message">
                  ...
                  <TableCell>
                    <div class="text-sm text-muted-foreground">
                      <div v-if="log.file">
                        ...
                        <span v-if="log.line" class="text-xs">:{{ log.line }}</span>
                      </div>
                      <span v-else>-</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <Link 
                      :href="route('error-logs.show', log.id)"
                ...
                <TableRow v-if="!errorLogs.data || errorLogs.data.length === 0">
                  <TableCell colspan="6" class="text-center py-8 text-muted-foreground">
                    Tidak ada riwayat error ditemukan
                  </TableCell>
                </TableRow>
              </TableBody>
            </Table>
          ...
          <!-- Pagination -->
          <div v-if="errorLogs.links && errorLogs.links.length > 3" class="mt-4 flex justify-center">
            <div class="flex space-x-1">
              <Link
                v-for="link in (errorLogs.links || [])"
                :key="link.label"
                :href="link.url"
                :class="[
                  'px-3 py-2 text-sm',
